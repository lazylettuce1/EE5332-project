...
architecture behave of matVecMul_control_s_axi is
    ...
    constant ADDR_ROW_SIZE_DATA_0 : INTEGER := 16#0010#;
    constant ADDR_ROW_SIZE_CTRL   : INTEGER := 16#0014#;
    constant ADDR_COL_SIZE_DATA_0 : INTEGER := 16#0018#;
    constant ADDR_COL_SIZE_CTRL   : INTEGER := 16#001c#;
    constant ADDR_A_BASE          : INTEGER := 16#2000#;
    constant ADDR_A_HIGH          : INTEGER := 16#3fff#;
    ...
    -- memory signals
    signal int_A_address0      : UNSIGNED(10 downto 0);
    signal int_A_ce0           : STD_LOGIC;
    signal int_A_q0            : UNSIGNED(31 downto 0);
    signal int_A_address1      : UNSIGNED(10 downto 0);
...
-- ----------------------- Instantiation------------------
-- int_A
int_A : matVecMul_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
...
-- ----------------------- Memory logic ------------------
    -- A
    int_A_address0       <= SHIFT_RIGHT(UNSIGNED(A_address0), 1)(10 downto 0);
    int_A_ce0            <= A_ce0;
    A_q0                 <= STD_LOGIC_VECTOR(SHIFT_RIGHT(int_A_q0, TO_INTEGER(int_A_shift0) * 16)(15 downto 0));
    int_A_address1       <= raddr(12 downto 2) when ar_hs = '1' else waddr(12 downto 2);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (A_ce0 = '1') then
                    int_A_shift0 <= UNSIGNED(A_address0(0 downto 0));
                end if;
            end if;
        end if;
    end process;
